 ...
 * Al crear/eliminar una devolución se actualiza el estado del pago asociado.
 */
@Controller
...
public class DevolucionController {
    ...
    /** GET /devoluciones/nueva — Formulario de nueva devolución */
    @GetMapping("/nueva")
    public String mostrarFormularioCrear(Model model, HttpSession session,
        ...
        // Solo pagos "realizado" sin devolución previa son candidatos a reembolso
    ...
    /** GET /devoluciones/editar/{id} — Formulario de edición */
    ...
    public String mostrarFormularioEditar(@PathVariable Integer id, Model model, HttpSession session) {
        ...
        // Al editar mostramos el pago actual de esta devolución (ya reembolsado)
        model.addAttribute("devolucion", devolucion.get());
        // Incluimos el pago actual para poder mostrarlo aunque esté "reembolsado"
        List<Pago> pagosReembolsables = pagoService.listarTodos().stream()
                .filter(p -> ("realizado".equals(p.getEstadoPago()) && p.getDevolucion() == null)
    ...
    /** POST /devoluciones/guardar — Guarda nueva devolución o actualiza */
    @PostMapping("/guardar")
